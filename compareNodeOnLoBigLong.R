...
hyperC <- read_csv("~/PaperDeebData/DeebDbLorenzBigTune/_summary/lorenz63std_NeuralOdeC.csv") |>
  mutate(
    methodBase = "NeuralOdeC",
    activation = "swish",
    weightDecay = 1e-6)
hyperB <- read_csv("~/PaperDeebData/DeebDbLorenzBigTune/_summary/lorenz63std_NeuralOdeB.csv") |>
  mutate(
    methodBase = "NeuralOdeB",
    activation = "swish",
    weightDecay = 0)
hyperBbb <- read_csv("~/PaperDeebData/DeebDbLorenzBigTune/_summary/lorenz63std_NeuralOdeB_bb.csv") |>
  mutate(
    methodBase = "NeuralOdeB_bb",
    activation = "swish",
    weightDecay = 0)
hyperCbb <- read_csv("~/PaperDeebData/DeebDbLorenzBigTune/_summary/lorenz63std_NeuralOdeC_bb.csv") |>
  mutate(
    methodBase = "NeuralOdeC_bb",
    activation = "swish",
    weightDecay = 1e-6)
hyperFbb <- read_csv("~/PaperDeebData/DeebDbLorenzBigTune/_summary/lorenz63std_NeuralOdeF_bb.csv") |>
  mutate(
    methodBase = "NeuralOdeF_bb",
    activation = "swish",
    weightDecay = 1e-6)
hyperE <- read_csv("~/PaperDeebData/DeebDbLorenzBigTune/_summary/lorenz63std_NeuralOdeE.csv") |>
  mutate(
...
scoresWithHyper <-
  s |>
  filter(model == "lorenz63std") |>
  left_join(bind_rows(hyperB, hyperC, hyperFbb, hyperBbb, hyperE, hyperCbb), join_by(methodBase, hash == id)) |>
